...
export class CreateUserProfileEvent {
  subject: Subjects.USER_PROFILE_CREATED = Subjects.USER_PROFILE_CREATED;
  data: {
    userId: string,
    firstName: string;
    lastName:string;
  ...
  constructor(data: { userId:string, firstName: string,lastName:string}) {
    this.data = data;
  }
}
